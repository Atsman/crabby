...
func RunSeleniumTest(j Job, seleniumServer string, storage *Storage) {
	...
	storage.MetricDistributor <- makeMetric(j.Name, j.URL, "dns_duration", wr.ri.dnsDuration)

	fmt.Println(j.Name, "Connection establishment time", wr.ri.serverConnectionDuration)
	storage.MetricDistributor <- makeMetric(j.Name, j.URL, "server_connection_duration", wr.ri.serverConnectionDuration)

	fmt.Println(j.Name, "Response time:", wr.ri.serverResponseDuration)
	storage.MetricDistributor <- makeMetric(j.Name, j.URL, "server_response_duration", wr.ri.serverResponseDuration)

	fmt.Println(j.Name, "Server processing time:", wr.ri.serverProcessingDuration)
	storage.MetricDistributor <- makeMetric(j.Name, j.URL, "server_processing_duration", wr.ri.serverProcessingDuration)
